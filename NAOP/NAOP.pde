...
int[] oldVirtualPosition;
int[] VirtualPosition;
int[] pos;
int[] oldPos;
int[] Pos; // to convert data of position always positively
int[] modPos; // ta have special position  quarter or half round
int[] modOldPos; //to have former
int[] modOldOldPos; //to have former
int[] TrigmodPos; // to trig accordinag modPos;
...
int[] PendularVirtualPosition;
int[] CircularVirtualPosition;
int[] DataToDueCircularVirtualPosition;
int[] ActualVirtualPosition;
int[] ActualVirtualPositionFromOtherMode;
int[] oldActualVirtualPosition;
int[] PendularOldVirtualPosition;
int[] CircularOldVirtualPosition;
int[] PendularLeftVirtualPosition;
int[] PendularOldLeftVirtualPosition;
int[] PendularOldOldLeftVirtualPosition;
int[] PendularOldOldOldLeftVirtualPosition;
...
//    DueSerialNativeUSBport101 = new Serial(this, Serial.list()[3], 9600);
...
//  printDataOnScreen();
 ...
 //  keyMode = " followDistribueAddLfoPatternControl " ;
  ...
  if (keyMode == " followDistribueAddLfoPattern " || keyModeRed == " followDistribueAddLfoPattern ") { //drive ball with lfo
...
for (int i = 0; i < networkSize; i++) {
 ...
 //   printMidiNoteVelocity();
  ...
  if (keyMode == " null ")
  {   
   if ( key =='a'||  key =='b' ||  key =='c' ||  key =='d' || key =='e' || key =='f' || key =='s' || key =='z'  ) // 
  {
  ...
  switch( modeStartKey) {
    ...
    text ( modeStartKeyToFollow, width/4, -height/4); 
    followSignalLfo(frameRatio, signal[11]);
    break;
    case 's':     
...
//  print ("formerKeyMetro "); 
  ...
  if (formerKeyMetro == 'J' && trigRatio == true ) { //drive ball with lfo ONCE 
...
void keyPressed() {
 ...
 if (keyCode == ALT){ // .. in Keypressed
  ...
  if ((key == '!'  ) ) {  
    print ("STOP MOVEMENT AND TIMER: "); //     // Toggle between sketch paused - running
    ...
    String dataMarkedToDue  ="<" 
      ...
      // +decompte[11]+","+decompte[10]+","+decompte[9]+","+decompte[8]+","+decompte[7]+","+decompte[6]+","+decompte[5]+","+decompte[4]+","+decompte[3]+","+decompte[2]+","+decompte[1]+","+decompte[0]+ ","
...
//**   /*
  ...
  text (counter11, -600, height +300); 
  text (counter2, -800, height +300); 
...
*/
 ...
 //************** REMETRRE
  ...
  for (int i = 1; i < 2; i++) {  // metroPhaseOnScreen
    ...
    String om = nf (metroPhase[11]-  metroPhase[10], 0, 3); 
...
//    firstFollowingStarted = true;
 ...
 //   net.oldPhase[i]=phaseMapped[i];
    phaseMapped[i]=phases[i-2][frameCount % nbMaxDelais]; // use varaible phaseMapped (to play movement with time delay or phase delay) to well send it in Teensy
 ...
 */
  ...
  */    
     ...
     println ( " net.phase11 ",  net.phase[11] , " movementInterpolated " , movementInterpolated );
 ...
 */
   ...
   for (int i = 2; i < networkSize-0; i+=1) {// display ball   (nbBall/2)-0; i < nbBall; i++   for (int i = 3; i < networkSize-0; i+=1) {
 //    print (net.oldPhase[i]); print (" 12448 ");   println (net.phase[i]); 
 //   net.oldPhase[i]=phaseMapped[i];
    phaseMapped[i]=phases[i-2][frameCountBis % nbMaxDelais]; // use varaible phaseMapped (to play movement with time delay or phase delay) to well send it in Teensy
  //  phaseMapped[i] =  phaseMapped[i]%TWO_PI; 
   // drawBallOppositeWay(  i, phases[i+0][frameCountBis % nbMaxDelais] );
   print ("  BEFBEF phaseMapped[i]  "); print (i); print ( " ");   println ( phaseMapped[i]  ); 
 ...
 */
  ...
  //  net.phase[i]=0;
    phaseMapped[i] = phases[i-2][frameCountBis % nbMaxDelais]+0; // to aligin ball with the followed one
...
void sendToTeensy() {
  ...
  if ( abs (speedi[11]) > 950 || abs (speedi[0]) > 950) {
    //   key = 'h'; keyReleased();
    textSize (200);
    text("CAREFULL", width/2, height - 20);
  }
...
void samplingMovementPro() {
 ...
 net.phase[networkSize-1]=  map (mouseY, 0, 400, 0, TWO_PI);
