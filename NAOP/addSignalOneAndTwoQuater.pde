...
// splitTimeQuater(30.0);
 ...
 //********POURQUOI DIFFERENT AU DEMARRAGE DE lA FONCTION
  if (oscillatorChanged==true && doZ==false )  { 
   ...
   if (doZ==true && ( oscillatorChange<=0 )){    // oscillatorChange<=0 || networkSize-1
       startZ=true;
   }

     if (oscillatorChanged==true && startZ==true  )  {   //
    ...
    if (startZ==true){ 
       ...
       LFO[oscillatorChanging] = phaseKeptAtChange[oscillatorChanging]+QUARTER_PI*1 ;  // on ajoute 
       dataMappedForMotor[oscillatorChanging]= (int) map (LFO[oscillatorChanging], TWO_PI, 0, 0, numberOfStep);  // le mappage est opposé
